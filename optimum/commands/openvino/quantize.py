...
"""Quantization with OpenVINO command-line interface class."""
...
def parse_args_openvino_quantize(parser: ArgumentParser):
    required_group = parser.add_argument_group("Required arguments")
    required_group.add_argument(
        "-m", "--model", type=Path, required=True, help="Model ID on huggingface.co or path on disk to load model from."
    )
    required_group.add_argument(
...
class OpenVINOQuantizeCommand(BaseOptimumCLICommand):
    @staticmethod
    def parse_args(parser: ArgumentParser):
        return parse_args_openvino_quantize(parser)
    ...
    def run(self):
        ...
        try:
            ...
            raise ImportError(
                "The OpenVINO quantization command requires `optimum-intel` to be installed. "
                "Please install it with: `pip install optimum[openvino]` or `pip install optimum-intel`"
            ) from e

        if self.args.output == self.args.model:
        ...
        if self.args.library is None:
            try:
                ...
                logger.warning(
                    "The library name could not be automatically inferred for the provided model. "
                    "Falling back to `transformers`."
                )
        else:
            library_name = self.args.library

        # text-generation models are transformers-based; sentence_transformers is not valid here.
        if library_name == "sentence_transformers" and str(self.args.task).startswith("text-generation"):
            logger.warning(
                "`--library sentence_transformers` is incompatible with text-generation tasks. "
                "Falling back to `transformers`."
            )
            library_name = "transformers"
